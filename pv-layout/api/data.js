...
// Fetches PV Unit Layout Reference v2 from Notion and returns clean JSON
...
// Map Notion property values to plain JS values
...
// Convert a raw Notion page into a clean widget-ready object
function mapRow(page) {
  const p = page.properties;
  const g = name => extractProp(p[name]);
  const num = name => g(name) ?? 0;

  const rooms = num('Bedrooms');
  const baths = num('Bathrooms');

  // Determine band from sqft
  ...
  return {
    ...
    // Dimension pairs [L_mm, W_mm] — 0 means not entered
    ...
    // Plan image URLs — null if not yet entered in Notion
...
// Paginate through all rows in the Notion database
async function fetchAllRows() {
  const rows = [];
  let cursor = undefined;

  do {
    const body = { page_size: 100, sorts: [{ property: 'Size (sqft)', direction: 'ascending' }] };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const data = raw
      .map(mapRow)
      .filter(r => r.project); // skip blank rows
